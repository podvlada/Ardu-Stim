...
// #if defined(ESP8266)
//   #define EEPROM_UPDATE(addr, value) EEPROM.write((addr), (value))
// #else
//   #define EEPROM_UPDATE(addr, value) EEPROM.update((addr), (value))
// #endif
...
// void saveConfig()
// {
//   EEPROM_UPDATE(EEPROM_WHEEL, config.wheel);
//   EEPROM_UPDATE(EEPROM_RPM_MODE, config.mode);
//   EEPROM_UPDATE(EEPROM_VERSION, VERSION);

//   byte highByte = highByte(currentStatus.rpm);
//   byte lowByte = lowByte(currentStatus.rpm);
//   EEPROM_UPDATE(EEPROM_CURRENT_RPM, highByte);
//   EEPROM_UPDATE(EEPROM_CURRENT_RPM+1, lowByte);

//   highByte = highByte(config.fixed_rpm);
//   lowByte = lowByte(config.fixed_rpm);
//   EEPROM_UPDATE(EEPROM_FIXED_RPM, highByte);
//   EEPROM_UPDATE(EEPROM_FIXED_RPM+1, lowByte);

//   highByte = highByte(config.sweep_low_rpm);
//   lowByte = lowByte(config.sweep_low_rpm);
//   EEPROM_UPDATE(EEPROM_SWEEP_RPM_MIN, highByte);
//   EEPROM_UPDATE(EEPROM_SWEEP_RPM_MIN+1, lowByte);

//   highByte = highByte(config.sweep_high_rpm);
//   lowByte = lowByte(config.sweep_high_rpm);
//   EEPROM_UPDATE(EEPROM_SWEEP_RPM_MAX, highByte);
//   EEPROM_UPDATE(EEPROM_SWEEP_RPM_MAX+1, lowByte);

//   highByte = highByte(config.sweep_interval);
//   lowByte = lowByte(config.sweep_interval);
//   EEPROM_UPDATE(EEPROM_SWEEP_RPM_INT, highByte);
//   EEPROM_UPDATE(EEPROM_SWEEP_RPM_INT+1, lowByte);

//   EEPROM_UPDATE(EEPROM_USE_COMPRESSION, config.useCompression);
//   EEPROM_UPDATE(EEPROM_COMPRESSION_TYPE, config.compressionType);
//   highByte = highByte(config.compressionRPM);
//   lowByte = lowByte(config.compressionRPM);
//   EEPROM_UPDATE(EEPROM_COMPRESSION_RPM, highByte);
//   EEPROM_UPDATE(EEPROM_COMPRESSION_RPM+1, lowByte);
//   highByte = highByte(config.compressionOffset);
//   lowByte = lowByte(config.compressionOffset);
//   EEPROM_UPDATE(EEPROM_COMPRESSION_OFFSET, highByte);
//   EEPROM_UPDATE(EEPROM_COMPRESSION_OFFSET+1, lowByte);
// #if defined(ESP8266)
//   EEPROM.commit();
// #endif
// }
